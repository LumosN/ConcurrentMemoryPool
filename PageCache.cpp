...
Span* PageCache::NewSpan(size_t k)
{
	...
	if (!_spanList[k].Empty()) // 要找的那个页数的span不为空，就直接拿
	{
		return _spanList[k].PopFront();
	}

	for (size_t i = k + 1; i < NPAGES; ++i)  // 为空就去找比它大的页，切开
	{
		// 大页给切小,切成k页的span返回
		// 切出i-k页挂回自由链表
		if (!_spanList[i].Empty())
		{
			...
			return split;  // return切出来的那个需要的那个span
		}
	}
...
void PageCache::ReleaseSpanToPageCache(Span* span)
{
	...
	while (1)
	{
		...
		//	break;
		//}
		Span* preSpan = _idSpanMap.get(preId);// 拿到前一个span
		if (preSpan == nullptr)
		{
			break;
		}
